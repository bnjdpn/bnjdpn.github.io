...
Contrôle ciblé complémentaire du 19 septembre, dans cette même propriété, sans demande d’indexation des fichiers XML : l’inspection en direct de `https://bnjdpn.github.io/sitemap.xml` a affiché « Google a accès à cette URL », « Exploration autorisée ? Oui » et « Récupération de page : Réussie » (test à 18:12:37). Le test en direct de `https://bnjdpn.github.io/sitemap-pages.xml` a donné les mêmes résultats (18:13:08). Ces tests prouvent une récupération par l’outil d’inspection, pas le traitement réussi par le système Sitemaps. Dans le rapport Sitemaps relu après les tests, chaque URL exacte restait de type « Inconnu », avec « Impossible de récupérer le sitemap », zéro page découverte et « Dernière lecture : 19/09/2026 » ; le détail de chaque ligne disait « Impossible de lire le sitemap », sans erreur plus précise. Les réponses publiques sont `200 application/xml` ; `robots.txt` autorise `/` et référence le sitemap principal ; celui-ci est un `sitemapindex` valide, ses 18 fichiers enfants sont servis en XML valide, et `sitemap-pages.xml` est un `urlset` valide contenant les trois guides, tous servis en HTTP 200. Aucun défaut du portfolio n’est démontré. La divergence entre l’inspection en direct et le rapport persiste : **cause non établie**. Ne pas soumettre à nouveau les sitemaps sur la base de ce seul état.
